...
def parseTile(tileInfo):
    ...
    # Extract Block Type
    block_type_match = re.search(r'tileType::(\w+)', tileInfo)
    blockType = block_type_match.group(1) if block_type_match else None

    # Extract First Coordinates (x1, y1)
    coord_matches = re.findall(r'\((\-?\d+),\s*(\-?\d+)\)', tileInfo)
    (llx, lly), (urx, ury) = [(int(x), int(y)) for x, y in coord_matches]  # Extract two coordinate pairs

    # Extract Width and Height
    wh_match = re.search(r'W=(\d+)\s+H=(\d+)', tileInfo)
    width, height = map(int, wh_match.groups()) if wh_match else (None, None)
