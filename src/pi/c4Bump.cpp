...
C4Bump::C4Bump(): PinMap("", 0, 0), m_c4BallOut(nullptr),
...
C4Bump::C4Bump(const std::string &fileName): m_c4BallOut(nullptr) {
    ...
    while(std::getline(file, lineBuffer)){
        ...
        if(splitLine[0] == "C4_END"){
            ...
            if(m_pinMapWidth <= 0){
                std::cout << "[PowerX:BalloutParser] Error: Basic attribute m_pinMapWidth not set or value(" << m_pinMapWidth << ") invalid" << std::endl;
            }
            if(m_pinMapHeight <= 0){
                std::cout << "[PowerX:BalloutParser] Error: Basic attribute m_pinMapHeight not set or value(" << m_pinMapHeight << ") invalid" << std::endl;
            }
            ...
            if(verifyCountWidth != m_pinMapWidth){
                std::cout << "[PowerX:BalloutParser] Error: Basic attribute mismatch m_pinMapWidth(" << m_pinMapWidth << ")";
            ...
            if(verifyCountHeight != m_pinMapHeight){
                std::cout << "[PowerX:BalloutParser] Error: Basic attribute mismatch m_pinMapHeight(" << m_pinMapHeight << ")";
                ...
                for (int i = 0; i < m_clusterCountWidth; ++i) {
                    ...
                    for(int n = 0; n < m_clusterPinCountHeight; ++n){
                        for(int m = 0; m < m_clusterPinCountWidth; ++m){
                            ...
                            this->cordToSignalTypeMap[pin] = st;
