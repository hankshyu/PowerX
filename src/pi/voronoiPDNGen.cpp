...
void VoronoiPDNGen::ripAndReroute(std::unordered_map<SignalType, std::vector<Cord>> &layerPoints, std::unordered_map<SignalType, std::vector<OrderedSegment>> &layerSegments){
    ...
    for(const OrderedSegment &cos : ripArr){
        ...
        std::cout << "[Rip And Reroute] Attempt BFS Routing From " << start << " -> " << goal << " of type: " << cosSt << " ";
...
void VoronoiPDNGen::generateInitialPowerPlanePointsX2(const std::unordered_map<SignalType, std::vector<Cord>> &inLayerPoints, const std::unordered_map<SignalType, std::vector<OrderedSegment>> &inLayerSegments,
...
void VoronoiPDNGen::generateVoronoiDiagramX(const std::unordered_map<SignalType, std::vector<FCord>> &flayerPoints, std::unordered_map<FCord, std::vector<FCord>> &voronoiCells){
...
void VoronoiPDNGen::mergeVoronoiCellsX(std::unordered_map<SignalType, std::vector<FCord>> &flayerPoints, std::unordered_map<FCord, std::vector<FCord>> &voronoiCellMap, 
...
void VoronoiPDNGen::enhanceCrossLayerPI(){
    ...
    for(int upLayerIdx = 0; upLayerIdx < (m_metalLayerCount-1); ++upLayerIdx){
        ...
        std::cout << "[Cross-Layer PI Enhancement] " << "Enhance PI between layer " << upLayerIdx << " & " << downLayerIdx << std::endl;
