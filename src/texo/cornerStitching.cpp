...
//  Module Name:        cornerStitching.cpp
//  Project Name:       PowerX
//  C++(Version):       C++17 
...
void CornerStitching::enumerateDirectedAreaRProcedure(const Rectangle &box, std::vector <Tile *> &allTiles, Tile *targetTile) const{
...
void CornerStitching::findLineTileHorizontalPositive(Tile *initTile, const Line &line, std::vector<LineTile> &positiveSide) const {

    assert(initTile != nullptr);
...
void CornerStitching::findLineTileHorizontalNegative(Tile *initTile, const Line &line, std::vector<LineTile> &negativeSide) const {
...
void CornerStitching::findLineTileVerticalPositive(Tile *initTile, const Line &line, std::vector<LineTile> &positiveSide) const {
...
void CornerStitching::findLineTileVerticalNegative(Tile *initTile, const Line &line, std::vector<LineTile> &negativeSide) const {
...
bool CornerStitching::checkPointInCanvas(const Cord &point, bool onlyLLCorners) const{
	return rec::isContained(mCanvasSizeBlankTile->getRectangle(), point, onlyLLCorners);
}
...
Tile *CornerStitching::findPoint(const Cord &key) const{
	...
	// make sure the point to be found (key) lies within canvas
    assert(checkPointInCanvas(key, true));
	...
	while(!(rec::isContained(index->getRectangle(), key, true))){
...
void CornerStitching::findLineTile(const Line &line, std::vector<LineTile> &positiveSide, std::vector<LineTile> &negativeSide) const {
	
	// check if the line is out of canvas range
    assert(checkPointInCanvas(line.getLow(), false));
    assert(checkPointInCanvas(line.getHigh(), false));
    ...
    assert(lineLowLegal && lineHighLegal);
...
bool CornerStitching::searchArea(const Rectangle &box) const{

    assert(checkRectangleInCanvas(box));
...
bool CornerStitching::searchArea(const Rectangle &box, Tile *someTile) const{
...
void CornerStitching::enumerateDirectedArea(const Rectangle &box, std::vector <Tile *> &allTiles) const{
	// modified by ryan: don't throw error, modify box to fit inside canvas
...
Tile *CornerStitching::insertTile(const Tile &tile){
    ...
    assert(checkRectangleInCanvas(tile.getRectangle()));

	// check if the tile inserting position already exist anotehr tile
    assert(!searchArea(tile.getRectangle()));
...
void CornerStitching::removeTile(Tile *tile){
    ...
    assert(deadTileIt != mAllNonBlankTilesMap.end());

	// the index does not point to the tile to delete
    assert(mAllNonBlankTilesMap[tileLL] == tile);
...
Tile *CornerStitching::cutTileHorizontally(Tile *origTop, len_t newDownHeight){
    ...
    assert(origTop->getHeight() > newDownHeight);
...
Tile *CornerStitching::cutTileVertically(Tile *origRight, len_t newLeftWidth){
    ...
    assert(origRight->getWidth() > newLeftWidth);
...
Tile *CornerStitching::mergeTilesVertically(Tile *mergeUp, Tile *mergeDown){
    ...
    assert(mergeUp->getYLow() > mergeDown->getYLow());
    ...
    assert(sameWidth && xAligned && tilesAttatched);
...
Tile *CornerStitching::mergeTilesHorizontally(Tile *mergeLeft, Tile *mergeRight){
    ...
    assert(mergeLeft->getXLow() < mergeRight->getXLow());
    ...
    assert(sameHeight && yAligned && tilesAttatched);
