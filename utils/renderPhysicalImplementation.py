...
def render_pdn_layer(
    ...
    no_legend: bool = True,
    no_title: bool = True,
    highlight_empty: bool = False,   # unused now
    verbose: bool = False,
):
    ...
    # Sizing from pxPerCell (prevents huge files)
    try:
    ...
    if node_patches:
        pc_nodes = PatchCollection(node_patches, facecolors=node_colors, edgecolors='none', alpha=0.95)
    ...
    for e in edges:
        ...
        if dist > 1e-6:
            shrink = r_node * 1.05  # a bit larger than radius for spacing
    ...
    if segs:
        lc = LineCollection(
            segs,
            colors=ecolors,
            linewidths=r_node * 0.3,   # much thinner than node radius
            alpha=0.8
        )
